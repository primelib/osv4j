...
public class OSVFactory {
    public static <T> T create(Consumer<OSVFactorySpec<T>> spec) {
        OSVFactorySpec<T> config = new OSVFactorySpec<>(spec);

        if (config.api().isInterface()) {
            return buildClient(spec);
        } else {
            try {
                OSVApi api = buildClient(s -> {
                    s.api(OSVApi.class);
                    s.applySpec(config);
                });
                return config.api().getConstructor(OSVApi.class).newInstance(api);
            } catch (Exception ex) {
                throw new IllegalArgumentException("api must have a constructor with one parameter of type OSVApi", ex);
            }
        }
    }

    public static OSVApi create() {
        return create(spec -> spec.api(OSVApi.class));
    }

    private static <T> T buildClient(Consumer<OSVFactorySpec<T>> spec) {
        OSVFactorySpec<T> config = new OSVFactorySpec<>(spec);
